...
_SYSTEM_PROMPT = (
    "You are a bearish equity researcher. Given technical, fundamental, and sentiment analysis, "
    "construct a compelling bearish case for the market and specific assets. "
    "Output ONLY valid JSON with keys: reasoning (str), thesis (str), conviction (float 0-1)."
)


def bear_researcher(state: AgentState, llm: OllamaClient, **kwargs: Any) -> dict[str, list[ResearcherOutput]]:
    logger.info("Bear researcher running...")
    ...
    prompt = (
        ...
        "Build the strongest bearish case given this data. "
        "Identify risks, overvaluation, and downside catalysts. "
        "Set conviction based on how strongly the data supports a bearish view (0.0 = no conviction, 1.0 = maximum)."
    )

    result = llm.generate_json(prompt, system=_SYSTEM_PROMPT, temperature=0.4)
    if "error" in result:
        result = {"reasoning": "Bear case unavailable", "thesis": "Neutral", "conviction": 0.5}

    if state.get("config", {}).get("verbose"):
        logger.info("Bear researcher thesis:\n%s", result.get("thesis", "")[:500])

    return {"bear_case": [ResearcherOutput(
